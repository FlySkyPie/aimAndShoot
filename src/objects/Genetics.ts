...
export class Genetics {
  public population: Player[] = [];
  public populationTmp: Player[] = [];
  ...
  createPopulation() {
    ...
    for (let i = 0; i < Constants.maxEnemies; i++) {
      const enemy = new Player(
        this.facade,
        Math.random() * Constants.w,
        Math.random() * Constants.h,
        Math.random() * Constants.TWOPI,
        this.getRandomColor(),
        true
      );

      enemy.brain = new Dejavu([6 * Constants.maxEnemies, 6, 7], 0.1, 100);
  ...
  evaluate() {
    let totalBulletsFired = this.facade.player.shootsFired;
    ...
    for (let i = 0; i < this.population.length; i++) {
      const agressive = this.divide(
        this.population[i].shootsFired,
        totalBulletsFired
      );

      const survial = this.divide(this.population[i].age, timeFacade.totalTime);

      const hits = this.divide(
        this.population[i].hits,
        this.population[i].shootsFired
      );

      const friendlyFire = this.divide(
        this.population[i].friendlyFire,
        this.population[i].shootsFired
      );
  ...
  crossOver(a: Player | null, b: Player | null) {
    if (!a) {
      a = new Player(
        this.facade,
        Math.random() * Constants.w,
      ...
      a.brain = new Dejavu([6 * Constants.maxEnemies, 6, 7], 0.1, 100);
    }
    ...
    if (!b) {
      ...
      b.brain = new Dejavu([6 * Constants.maxEnemies, 6, 7], 0.1, 100);
    }

    const color: [number, number, number] = [0, 0, 0];

    for (let i = 0; i < color.length; i++) {
      if (Math.random() < 0.5) color[i] = (a.color[i] + b.color[i]) / 2;
      else color[i] = Math.random() < 0.5 ? a.color[i] : b.color[i];
    }
    ...
    child.brain = new Dejavu([6 * Constants.maxEnemies, 6, 7], 0.1, 100);
  ...
  mutate(child: Player) {
    for (let i = 0, end = Math.floor(Math.random() * 3); i < end; i++) {
      child.color[Math.floor(Math.random() * 3)] = Math.floor(
        Math.random() * 256
      );
    }
